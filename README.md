...
```ts
cordova.plugins.firebase.messaging.requestPermission({forceShow: false, provisional: false}).then(function() {
...
| `options.provisional` | `boolean` | (iOS only) When value is `true` only provisional notification permissions are requested. |
